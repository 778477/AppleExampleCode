...
- (void)startWithHostAddressOnOtherRunLoop{
...
- (void)start {
    [self.class startCFSimplePingThreadIfNeed];
    
    [self performSelector:@selector(scheduleHost)
                 onThread:simplePingThread
               withObject:self
            waitUntilDone:YES];
}


NSString *const kSimplePingThreadName = @"com.apple.SimplePing";
+ (void)simplePingThreadRun{
    NSThread *currentThread = [NSThread currentThread];
    NSRunLoop *currentRunLoop = [NSRunLoop currentRunLoop];
    currentThread.name = kSimplePingThreadName;
...
static NSThread *simplePingThread;

+ (void)startCFSimplePingThreadIfNeed{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        simplePingThread = [[NSThread alloc] initWithTarget:self selector:@selector(simplePingThreadRun) object:nil];
        [simplePingThread start];
    });
}
...
- (void)stopHostResolutionAndSocket{
    [self stopHostResolution];
    [self stopSocket];
}

- (void)stop {
    [self performSelector:@selector(stopHostResolutionAndSocket)
                 onThread:simplePingThread
               withObject:nil
            waitUntilDone:YES];
